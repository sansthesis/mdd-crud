...
public class GeneratedResourceEmitterTest {
  ...
  public void testGenerateModel() throws Exception {
    ...
    final Model model = scanner.generateModel(Division.class);
    final Emission out = emitter.emit(model);
    Assert.assertEquals("com.github.jasonrose.crud.om.generated.GeneratedDivisionResource", out.getFilename());
    final String verify = CharStreams.toString(Resources.newReaderSupplier(Thread.currentThread().getContextClassLoader().getResource("GeneratedDivisionResource.java"), Charsets.UTF_8));
